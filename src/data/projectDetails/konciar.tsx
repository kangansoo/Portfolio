...
export const konciarData: ProjectDetailData = {
  ...
  intro:
    "한국의 통신/금융 기반 본인 인증과 언어 장벽으로 인해 국내 로컬 서비스 이용에 어려움을 겪는 외국인 여행객을 위한 컨시어지 서비스 \n창업 팀의 개발 역할로 참여하였고, 프로토타입 개발 완료 후 바이럴 마케팅과 길거리 인터뷰 등, 개발 외적인 업무에도 주도적으로 참여하며 행동하는 개발자가 되고자 경험을 쌓았습니다.",
  projectInfo: [
    {
  ...
  features: [
    ...
    {
      title: "이메일 브릿지 시스템",
      desc: "서버 비용 없는 프로토타입 개발로 실시간 메일 전송 환경 구축",
    },
  ],
  roles: [
    {
      ...
      problem: [
        "단일 HTML 파일 코드베이스는 유지보수성·가독성·확장성 모두 한계",
    ...
    },
    {
      ...
      solution: [
        "**i18next & i18next-http-backend 도입**: 번역 데이터를 소스 코드와 완전히 분리하고, 필요한 언어 팩만 런타임에 비동기로 로드하는 구조",
  ...
  achievements: [
    ...
    "**글로벌 대응 역량**: 다국어 지원 라이브러리를 적용하여 글로벌 사용자를 고려한 서비스 설계 및 구현 경험",
