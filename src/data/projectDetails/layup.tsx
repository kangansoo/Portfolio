...
export const layupData: ProjectDetailData = {
  ...
  roles: [
    ...
    },
    {
      ...
      problem: ["이미지 리스트 로딩 시 발생하는 네트워크 병목 현상과 대규모 이미지 관리의 어려움", "이미지 관련 태그 없이는 AI가 어떤 이미지를 선택할지 사용자가 예측할 수 없어 서비스 신뢰도 저하"],
      solution: [
        "**React-Query 캐싱**: 캐싱 전략과 무한 스크롤을 조합하여 대규모 자산의 중복 호출을 방지",
        "**스켈레톤 UI**: 콘텐츠 로드 전 레이아웃을 미리 확보하여 데이터 로딩 중 시각적 레이아웃 안정성 및 체감 대기 시간 개선",
        "**이미지 태그 시스템**: 이미지 업로드 시 부여된 설명 태그 확인 및 수정 가능",
        "**SSR/CSR 분리**: 소상공인이 검색으로 서비스를 발견하는 경로를 고려해 SSR로 SEO 확보 및 초기 로딩 속도 개선, CSR로 동적 콘텐츠 렌더링 분리",
      ],
      result: [
        "이미지 로딩 성능 최적화를 통해 리소스 탐색 속도를 개선하고 시각적으로 안정적인 UX 제공",
        "태그 기반 이미지 선택 구조 도입으로 AI 자동 선택의 불확실성 해소 — 사용자가 포스팅 결과를 사전에 예측 가능 및 개입 가능한 구조 확보",
        "SSR 적용으로 소상공인의 검색 경로를 통한 서비스 진입 가능성 확보 (SEO 개선)",
      ],
    },
    {
      ...
      problem: ["소상공인 매장 대부분에 PC 미비 — 촬영 후 PC로 이미지를 옮기는 과정이 번거로워 모바일에서 바로 업로드·관리할 수 있어야 함", "별도 앱 개발 없이 모바일 환경에서 앱 수준의 경험을 제공해야 하는 구현 제약"],
      solution: [
      ...
      result: "현장에서 촬영한 이미지를 모바일로 바로 업로드·관리할 수 있는 환경 구축 — PC 없이도 서비스 전체 운영 가능한 접근성 확보",
    },
    {
      ...
      problem: "소상공인 매장 대부분에 PC 미비 — 메뉴 변동 시 스마트폰으로 즉시 처리할 수 있어야 하며, PC를 찾아가야 하는 흐름은 현장 운영자에게 비현실적",
      solution: ["**PWA 기반 실시간 관리**: 별도의 스토어 설치 없이 홈 화면 앱을 통해 현장에서 메뉴 품절이나 가격 변동 사항을 즉각적으로 처리할 수 있는 높은 접근성 확보"],
      result: [
        "모바일 최적화 인터페이스와 실시간 데이터 동기화를 통해 오프라인 사업체의 운영 효율성 향상",
        "PWA 기반으로 별도 앱 스토어 설치 없이 현장에서 즉시 접근 가능한 메뉴 관리 환경 구축",
        "메뉴 변동사항(품절, 가격 수정 등)을 스마트폰으로 즉각 처리 가능하여 PC 없이도 사업체 운영 지속 가능",
      ],
    },
  ],
  achievements: [
    ...
    "**AI 보조 도구 활용**: Cursor AI 도입으로 일정 내 PWA 추가 구현 — 리소스 제약 속에서도 사용자 접근성 개선 달성",
    "**코드 품질 향상**: 관심사를 명확히 분리하고 복잡한 비즈니스 로직을 Custom Hooks로 캡슐화하여 코드의 재사용성과 유지보수성을 극대화",
    "**소상공인 사용 시나리오 기반 설계**: 촬영 → 모바일 드라이브 업로드 → PC 템플릿 설정으로 이어지는 역할 분리를 통해 PC 없이도 서비스 전체 운영 가능한 접근성 확보",
  ],
}
