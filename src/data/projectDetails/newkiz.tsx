  ...
  roles: [
    {
      ...
      solution: [
        "**FSD(Feature-Sliced Design) 채택**: 뉴스·퀴즈·실시간 게임·AI 챗봇 등 독립적으로 발전해야 하는 기능들을 모듈 단위로 분리 — 신규 기능 추가나 데이터 재사용 시에도 다른 기능에 영향 없는 구조 확보",
    ...
    },
    {
      ...
      problem: [
        ...
        "다양한 디바이스 해상도 차이로 인한 캐릭터 위치 불일치 — 데스크탑 환경에서 전송된 절대 좌표 값이 모바일 환경에서 맵의 가동 범위를 벗어나는 현상",
      ...
      result: [
        "모든 참여 기기에서 동일한 게임 화면 보장 — 기기·해상도에 따른 불공정 경험 제거",
        "실시간 OX 퀴즈 게임 참여 기기 간 좌표 정합성 확보",
        "이동 거리 임계값 + 3프레임 주기 기반 전송 로직으로 위치 전송 빈도를 줄여 네트워크 트래픽 부하 약 60% 절감 및 끊김 없는 게임 경험 제공",
      ],
    },
    {
      title: "카테고리 시스템 고도화",
      ...
      problem: "초기 구조에서 대분류 카테고리를 텍스트만으로 나열 — 아이콘 등 시각적 단서 없이 텍스트만으로 구성되어 어린이 사용자가 관심 세부 주제로 접근하는 경로가 직관적이지 않음",
      solution: ["**계층형 카테고리 설계**: 주요 뉴스 사이트 구조를 참고하여 대분류 → 어린이 친화적 아이콘을 갖춘 세부 카테고리로 이어지는 2단계 탐색 구조로 개선"],
      result: [
        "정보 구조 최적화를 통해 어린이 사용자의 뉴스 탐색 용이성 및 콘텐츠 도달률 개선",
        "대분류 → 아이콘 포함 세부 카테고리의 2단계 구조 도입으로 어린이 사용자가 관심 주제에 직관적으로 도달 가능",
        "시각적 단서(아이콘) 추가로 텍스트 읽기 전에 카테고리 식별 가능 — 타겟 사용자층(어린이)의 리터러시 수준을 고려한 UX 개선",
      ],
    },
  ],
  achievements: [
